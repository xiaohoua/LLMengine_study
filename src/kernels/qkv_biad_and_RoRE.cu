...
//生成旋转矩阵 // rot_embed_dim (d), base(10000), t_step(m)
inline __device__ float2 GetRoPEfreq(int zid, int rot_embed_dim,
                                     float base, float t_step)
{
...
// // zid在这里获取
// inline __device__ void apply_RoPE(half2 &q, half2 &k, int tid, int rot_embed_dim,
//                                     float base, float t_step)
// {
//     // zid = 2*tid,每个线程处理两个half
...
// inline __device__ void apply_RoPE(float4 &q, float4 &k, int tid, int rot_embed_dim,
//                                     float base, float t_step)
// {
